...
def run_segment(img, hst, thresh=None):
    hs, sv = get_hs(img)
    bc = hst.shape[0]
    ri = np.clip((hs * (bc - 1)).astype(int), 0, bc - 1)
    gi = np.clip((sv * (bc - 1)).astype(int), 0, bc - 1)
    probs = hst[ri, gi]
    ...
    overlay = img.copy()
    overlay[best_mask == 0] = 0

    return best_mask * 255, overlay

def main():
    tr, te, od = 'training', 'testing', 'results_mp3_hs'
    ...
    if os.path.exists(te):
        for fn in [f for f in os.listdir(te) if f.lower().endswith('.bmp')]:
            ...
            ax[1].imshow(overlay); ax[1].set_title("Skin Segmentation"); ax[1].axis('off')
            plt.tight_layout()
